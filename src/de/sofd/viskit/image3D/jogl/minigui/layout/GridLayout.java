...
public class GridLayout extends Layout
{
                                    ...
                                    int height )
    {
        ...
        for ( int rh : rowHeight ) prefHeight += rh;
        for ( int cw : colWidth ) prefWidth += cw;
        
        return new Size( prefWidth, prefHeight );
    }

    @Override
    public void pack( int x, int y, int width, int height )
    {
        int[] colWidth = new int[columns];
        int[] rowHeight = new int[rows];
        ...
        for ( int row = rows - 1 ; row >= 0; --row )
        {
            ...
            for ( int col = 0; col < columns; ++col )
            {
                ...
                if ( comp != null ) 
                {
                    comp.setSize( cx, cy, colWidth[col], rowHeight[row] );
                    comp.pack();
                }
